...
C = wrap(re.sub('[^A-Za-z]+', '', C), 12)


# print(C)


def calc_letter(char):
...
def prettify(list, d, nr):
    ...
    # for x in list:
    #    x = x.lower()
    # print(str(calc_letter(list[0])) + "  ", end='')
...
def dec(x, i):
    return chr((calc_letter(x) + K[i]) + 96)
...
# print(calc_letter('f'),calc_letter('l') ,calc_letter('a'),calc_letter('g'))
